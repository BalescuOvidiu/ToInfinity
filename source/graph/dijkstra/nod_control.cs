...
public class nod_control : MonoBehaviour {
    ...
    public muchie_control[] muchiiA = new muchie_control[100];
    public muchie_control[] muchiiB = new muchie_control[100];
    ...
    void OnMouseDown()
    {
        if (MS.mutaNodActive && !MS.startDijkstra)
        {
            for (int i = 1; i <= MS.q; i++)
            {
                ...
                if (muchiiObjA[i] != null)
                    muchiiA[i] = muchiiObjA[i].GetComponent<muchie_control>();
                if (muchiiObjB[i] != null)
                    muchiiB[i] = muchiiObjB[i].GetComponent<muchie_control>();
            }
            isDragging = true;
        }
        if(MS.stergeNodActive && !MS.startDijkstra)
        {
            ...
            for(int i=1;i<=MS.q;i++)
            {
                MS.graf[numar][i] = MS.graf[i][numar] = 0;
	...
	void OnMouseDrag()
    {
        if (!MS.mutaNodActive)
            drag = true;
        else if (MS.mutaNodActive && MS.isInRect() && MS.ok && !MS.startDijkstra) 
        {
